	...
	public static function label($label,$for,$htmlOptions=array())
	{
		...
		if(isset($htmlOptions['required']))
		{
			if($htmlOptions['required'])
			{
				...
				// author 胡尚明
				// 如果是required的字段，yii默认会加*，增加控制：
				// 如果传入了_afterRequired则使用传入的_afterRequired，否则使用默认的$afterRequiredLabel
				$_afterRequiredLabel = self::$afterRequiredLabel;
				if(isset($htmlOptions['_afterRequired'])){
					$_afterRequiredLabel = $htmlOptions['_afterRequired'];
				}
				
				$label=self::$beforeRequiredLabel.$label.$_afterRequiredLabel;
			}
		...
		//author胡尚明
		//注释上一句返回，tag标签修改，如果传入了指定的tag，则使用指定tag，否则使用默认的label标签
		$_lableName = 'label';
		if(isset($htmlOptions['_label'])){
			$_lableName = $htmlOptions['_label'];
			unset($htmlOptions['_label']);
		}
		return self::tag($_lableName,$htmlOptions,$label);
	}
